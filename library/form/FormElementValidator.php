...
class Library_Form_FormElementValidator
{
	public static function validateRule($ruleName, $ruleValue, $elementValue)
	{
		...
		switch($ruleName)
		{
			case(Library_Form_ValidationRuleConst::REQUIRED):
				$isValid = self::validateRequiredField($ruleValue, $elementValue);
				break;
			case(Library_Form_ValidationRuleConst::FORMAT):
	...
	private static function validateFormat($ruleValue, $elementValue)
	{
		...
		switch($ruleValue)
		{
			case(Library_Form_ValidationRuleConst::NUMERIC_FORMAT):
				$isValid = preg_match('/^[1-9]*$/', $elementValue);
				break;
				
			case(Library_Form_ValidationRuleConst::ALPHABETICAL_FORMAT):
				$isValid = preg_match('/^[A-Za-z]*$/', $elementValue);
				break;
				
			case(Library_Form_ValidationRuleConst::ALPHANUMERIC_FORMAT):
