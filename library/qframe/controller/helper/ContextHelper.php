...
class Library_Qframe_Controller_Helper_ContextHelper
{
	...
	private function applyView(Library_Qframe_Controller_BaseController $controller)
	{
		...
		if(empty($module))
		{
			$viewPath = PROJECT_PATH . "/application/views/scripts/" . $requestedController . "/" . $action . ".php";
		}
		else
		{
			$viewPath = PROJECT_PATH . "/application/modules/" . $module . "/views/scripts/" . $requestedController . "/" . $action . ".php";
		}
	...
	private function applyJSONConversion(Library_Qframe_Controller_BaseController $controller)
	{
		...
		foreach($jsonView as $key => $value)
		{
			if(is_object($value))
			{
				if(get_parent_class($value) == 'Library_Qframe_Model_BaseItem')
				{
					$jsonView[$key] = $value->getPrinter()->JSONPrint();
				}
			}
		}
		
		return(json_encode($jsonView));
	}
