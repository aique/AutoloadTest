...
class Library_Html_Validation_FormElementValidator
{
	private $form;
	
	public function __construct(Library_Html_Element_BaseForm $form)
	{
		$this->form = $form;
	}
	
	public function validateElement(Library_Html_Element_FormElement $element)
	{
	...
	private function validateRule($ruleName, $ruleValue, $elementValue)
	{
		...
		switch($ruleName)
		{
			case(Library_Html_Const_ValidationRuleConst::REQUIRED):
				$isValid = self::validateRequiredField($ruleValue, $elementValue);
				break;
				
			case(Library_Html_Const_ValidationRuleConst::FORMAT):
